...
| Rodion / Hong Lu / Ryoshu / Sinclair core passives | `PARTIAL` | Hong Lu's `Kōzan` follow-up, the `[Bright -光-]` clauses and the "cancel the Skill" gate are in; Live: Rodion's Blessing / Despair state (with the statuses' own upkeep), Ryoshu's Petals from Sinking damage and Tremor Bursts **plus her three Unopposed Attacks** ("Bang. Bang.", "Stories that Never Cease", "Magnificent End"), Hong Lu's Hand conversion (deferred to the next Turn Start), Gregor's Speed-free redirect and Outis' Sheut Fracture rider.  Open: the Plus/Minus Coin Skill replacement itself (the identity data carries no alternate coin values), Sinclair's Evade-driven Stagger-threshold reduction is in; what remains is the Plus/Minus Coin replacement only. |
