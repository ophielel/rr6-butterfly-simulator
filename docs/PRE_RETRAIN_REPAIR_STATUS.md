...
- Backend parity, strict critical preflight, multi-hit Sinking audit, and the human setup oracle are complete. A follow-up audit found the first PPO training run omitted `infinite_ego_resources` in both rollout and validation resets; that run is superseded. The reward audit also fixed Boss HP credit to use actual Imago HP delta rather than aggregate butterfly damage. Scenario-specific Teacher → BC → PPO runs are recorded in `reports/ppo_hp_scenario_specific_1000.json`: 200 Teacher episodes and 1000 PPO rollouts per scene. Matched evaluation reached 27/100 for half HP and 0/100 for full HP. Full HP Teacher itself produced 0/200 wins, so the full-HP result is currently a reachability limitation, not evidence that more PPO iterations would solve it.
