...
4. **奖励修订**：早期实验曾将 `sinking_damage` 以 `+0.02` 加入每回合奖励；这会把沉沦触发本身变成训练目标。当前实现只奖励真实 Imago HP delta，`sinking_damage` 仅保留为统计字段。旧结果必须标为 legacy 或 pre-reward-removal，不能和修订后结果混合。
5. **计算预算**：教师数据 400 局、BC 若干轮、PPO 400 局、测试集 100 个独立 seed。
